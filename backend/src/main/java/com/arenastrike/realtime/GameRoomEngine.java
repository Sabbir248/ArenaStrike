...
public final class GameRoomEngine implements AutoCloseable {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(GameRoomEngine.class);

    private final String roomId;
    private final GameRoomState room;
    private final SimpMessagingTemplate messagingTemplate;
    private final ConcurrentLinkedQueue<PlayerInputEvent> inputQueue = new ConcurrentLinkedQueue<>();
    ...
    public GameRoomEngine(String roomId, GameRoomState room, SimpMessagingTemplate messagingTemplate,
    ...
    public void enqueueInput(PlayerInputCommand input) {
        if (isMatchTerminated.get() || room.isFinished()) {
            return;
        }
        inputQueue.offer(new PlayerInputEvent(input));
    }

    public void removePlayer(Long playerId) {
        inputQueue.removeIf(input -> input.playerId().equals(playerId));
    ...
    private void tick() {
        try {
            ...
            if (snapshot.roomState() == RoomState.ACTIVE
                    && snapshot.remainingSeconds() != lastTimerSecond) {
                lastTimerSecond = snapshot.remainingSeconds();
                messagingTemplate.convertAndSend("/topic/room/" + roomId + "/timer",
                        new RoomTimerEvent(roomId, lastTimerSecond));
            }
            finishMatch(room.finishIfExpired());
